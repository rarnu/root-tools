...
public class YugiohUtils {
		...
		if (c != null) {
			...
			while (!c.isAfterLast()) {
				result.add(c.getString(c
						.getColumnIndex(FieldDefine.EffectFields[2])));
				c.moveToNext();
			}
		}
	...
	public static Cursor getCards(Context context, String cardType,
		...
		return context
				...
				.query(ContentUris.withAppendedId(YugiohProvider.CONTENT_URI,
						...
						new String[] { FieldDefine.DataFields[0],
								FieldDefine.DataFields[5],
								FieldDefine.DataFields[10] }, where, args, null);
	...
	public static Cursor getAssignedCards(Context context, String union) {
		...
		return context
				...
				.query(ContentUris.withAppendedId(YugiohProvider.CONTENT_URI,
						...
						new String[] { FieldDefine.DataFields[0],
								FieldDefine.DataFields[5],
								FieldDefine.DataFields[10] }, where, null, null);
	}
	...
	public static CardInfo cursorToCardInfo(Cursor c) throws Exception {
		...
		Method mSet = null;
		for (Field f : fs) {
			mSet = cls.getMethod("set" + f.getName(),
					new Class[] { f.getType() });
			if (f.getType().getName().contains("String")) {
				mSet.invoke(info, new Object[] { c.getString(c.getColumnIndex(f
						.getName())) });
			} else {
				mSet.invoke(
						info,
						new Object[] { c.getInt(c.getColumnIndex(f.getName())) });
			}
		}
