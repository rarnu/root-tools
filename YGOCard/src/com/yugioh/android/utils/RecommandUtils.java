...
public class RecommandUtils {
	...
	public static void showTextRecommand(Context context, RecommandInfo info) {
		...
		if (!info.bigQR.equals("")) {
			...
			ab.setView(ivQr);
			DownloadUtils.downloadFileT(context, ivQr,
					NetworkDefine.RECOMMAND_IMAGE_URL + info.bigQR,
					PathDefine.RECOMMAND_PATH, info.bigQR, null);
		}
	...
	private static void jumpUrlQr(final Context context,
			final RecommandInfo info) {
		AlertDialog.Builder ab = new AlertDialog.Builder(context);
		ab.setTitle(info.name);
		ab.setMessage(info.jumpText);
