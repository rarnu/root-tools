...
- (void)initializer {
    ...
    self.colorLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 10, self.frame.size.height + 5)];
    self.colorLabel.backgroundColor = [UIColor orangeColor];
    
    [self.cellScrollView addSubview:self.colorLabel];
    ...
    self.leftUtilityButtonsView = [[SWUtilityButtonView alloc] initWithUtilityButtons:nil parentCell:self utilityButtonSelector:@selector(leftUtilityButtonHandler:)];
    ...
    self.rightUtilityButtonsView = [[SWUtilityButtonView alloc] initWithUtilityButtons:nil parentCell:self utilityButtonSelector:@selector(rightUtilityButtonHandler:)];
...
- (void)updateCellState {
    if(layoutUpdating == NO) {
        for (NSNumber *numState in @[@(kCellStateCenter),@(kCellStateLeft),@(kCellStateRight),]) {
