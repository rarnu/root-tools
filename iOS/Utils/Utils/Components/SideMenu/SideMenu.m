...
- (void)__presentMenuViewContainerWithMenuViewController:(UIViewController *)menuViewController {
    ...
    if ([self.delegate conformsToProtocol:@protocol(SideMenuDelegate)] && [self.delegate respondsToSelector:@selector(sideMenu:willShowMenuViewController:)]) {
...
- (void)__showLeftMenuViewController {
    ...
    [UIView animateWithDuration:self.animationDuration animations:^{
        ...
        if (!self.visible && [self.delegate conformsToProtocol:@protocol(SideMenuDelegate)] && [self.delegate respondsToSelector:@selector(sideMenu:didShowMenuViewController:)]) {
...
- (void)__showRightMenuViewController {
    ...
    [UIView animateWithDuration:self.animationDuration animations:^{
        ...
        if (!self.rightMenuVisible && [self.delegate conformsToProtocol:@protocol(SideMenuDelegate)] && [self.delegate respondsToSelector:@selector(sideMenu:didShowMenuViewController:)]) {
...
- (void)__hideMenuViewControllerAnimated:(BOOL)animated {
    ...
    if ([self.delegate conformsToProtocol:@protocol(SideMenuDelegate)] && [self.delegate respondsToSelector:@selector(sideMenu:willHideMenuViewController:)]) {
    ...
    void (^completionBlock)(void) = ^{
        ...
        if (!strongSelf.visible && [strongSelf.delegate conformsToProtocol:@protocol(SideMenuDelegate)] && [strongSelf.delegate respondsToSelector:@selector(sideMenu:didHideMenuViewController:)]) {
...
- (void)__panGestureRecognized:(UIPanGestureRecognizer *)recognizer {
    if ([self.delegate conformsToProtocol:@protocol(SideMenuDelegate)] && [self.delegate respondsToSelector:@selector(sideMenu:didRecognizePanGesture:)])
    ...
    if (recognizer.state == UIGestureRecognizerStateChanged) {
       ...
       if (!self.bouncesHorizontally && self.visible) {
        ...
        if (!self.didNotifyDelegate) {
            if (point.x > 0) {
                if (!self.visible && [self.delegate conformsToProtocol:@protocol(SideMenuDelegate)] && [self.delegate respondsToSelector:@selector(sideMenu:willShowMenuViewController:)]) {
