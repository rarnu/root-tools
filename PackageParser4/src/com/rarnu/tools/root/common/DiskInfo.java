...
public class DiskInfo {
    ...
    public String getColoredSize(int type) {
        ...
        switch (type) {
            case 0:
                percent = DiskUtils.stringToKbytes(used) / DiskUtils.stringToKbytes(size);
                if (percent <= 0.6D) {
                    colorStr = "#00FF00";
                ...
                break;
            case 1:
                percent = DiskUtils.stringToKbytes(free) / DiskUtils.stringToKbytes(size);
                if (percent >= 0.4D) {
                    colorStr = "#00FF00";
        ...
        return String.format("<font color=%s>%s</font>", colorStr, (type == 0 ? used : free));
    }
}
