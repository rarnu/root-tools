...
public class InnerListView extends InnerAbsListView {
	...
	public InnerListView(Context context, AttributeSet attrs, int defStyle) {
		...
		final Drawable osHeader = a
				.getDrawable(R.styleable.ListView_overScrollHeader);
		if (osHeader != null) {
			setOverscrollHeader(osHeader);
		}

		final Drawable osFooter = a
				.getDrawable(R.styleable.ListView_overScrollFooter);
		if (osFooter != null) {
	...
	public void setAdapter(ListAdapter adapter) {
		...
		if (mHeaderViewInfos.size() > 0 || mFooterViewInfos.size() > 0) {
			mAdapter = new InnerHeaderViewAdapter(mHeaderViewInfos,
	...
	protected int getItemTop(int pos) {
		...
		return count > 0 ? getChildAt(count - 1).getBottom() + mDividerHeight
				: getListPaddingTop();
	}

	protected int getItemBottom(int pos) {
		int count = getChildCount();
		return count > 0 ? getChildAt(0).getTop() - mDividerHeight
	...
	private void setupChild(View child, int position, int y, boolean flowDown,
		...
		InnerAbsListView.LayoutParams p = (InnerAbsListView.LayoutParams) child
				.getLayoutParams();
		if (p == null) {
			p = new InnerAbsListView.LayoutParams(
		...
		if ((recycled && !p.forceAdd)
				|| (p.recycledHeaderFooter && p.viewType == InnerAdapterView.ITEM_VIEW_TYPE_HEADER_OR_FOOTER)) {
	...
	private void scrollListItemsBy(int amount) {
		...
		if (amount < 0) {
			...
			while (first.getBottom() < listTop) {
				InnerAbsListView.LayoutParams layoutParams = (LayoutParams) first
						.getLayoutParams();
			...
			while (last.getTop() > listBottom) {
				InnerAbsListView.LayoutParams layoutParams = (LayoutParams) last
						.getLayoutParams();
				if (recycleBin.shouldRecycleViewType(layoutParams.viewType)) {
	...
	protected void dispatchDraw(Canvas canvas) {
		...
		if (drawDividers || drawOverscrollHeader || drawOverscrollFooter) {
			...
			final ListAdapter adapter = mAdapter;
			final boolean fillForMissingDividers = drawDividers && isOpaque()
					&& !super.isOpaque();
