...
public abstract class BaseService extends Service {
    ...
    private void operation(final String command, final int id, final int title, final int desc, final int proc_id, final int proc_title, final int proc_desc) {

        final Notification n = NotificationUtils.buildNotification(getApplicationContext(), id, getIcon24(), proc_title, proc_desc, Actions.ACTION_NOTIFY_NULL, null, false);
        if (showNotification()) {
    ...
    private void doNotification(int id, int title, int desc, boolean canClose, String action) {
        NotificationUtils.showNotification(getApplicationContext(), id, getIcon24(), title, desc, action, canClose);
    }
        ...
        if (intent != null) {
            ...
            if (command != null) {
                if (getCommandCondition(command)) {
                    ...
                    initIntent();
                    doSendMessage();
                    operation(command, id, title, desc, proc_id, proc_title, proc_desc);
                }
            }
        }
