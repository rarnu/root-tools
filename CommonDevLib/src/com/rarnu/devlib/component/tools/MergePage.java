...
public class MergePage {
	...
	public BlockView[] getViews(Context context, int layout, String[] keys,
			int[] ids, BitmapFactory.Options option, FocusCallback callback) {
	...
	private void parseExtra(View v, String[] keys, int[] ids, String extra,
				...
				if (subView instanceof ImageView) {
					...
					if (uri.startsWith("http://") || uri.startsWith("https://")) {
						...
						int imgRes = context.getResources().getIdentifier(
								uri.replace("resource://", ""), "drawable",
								context.getPackageName());
						((ImageView) subView).setImageBitmap(BitmapFactory
								.decodeResource(context.getResources(), imgRes,
										option));
					} else {
						// load via local file
						((ImageView) subView).setImageBitmap(BitmapFactory
