...
import com.rarnu.devlib.component.intf.DragListener;
import com.rarnu.devlib.component.intf.DragScrollProfile;
import com.rarnu.devlib.component.intf.DropListener;
	...
	public DragListView(Context context, AttributeSet attrs) {
		...
		if (attrs != null) {
			TypedArray a = getContext().obtainStyledAttributes(attrs,
					R.styleable.DragListView, 0, 0);

			mItemHeightCollapsed = Math.max(1, a.getDimensionPixelSize(
					R.styleable.DragListView_collapsed_height, 1));

			mFloatAlpha = a.getFloat(R.styleable.DragListView_float_alpha,
					mFloatAlpha);
			mCurrFloatAlpha = mFloatAlpha;

			mDragEnabled = a.getBoolean(R.styleable.DragListView_drag_enabled,
					mDragEnabled);

			mSlideRegionFrac = Math.max(0.0f, Math.min(1.0f, 1.0f - a.getFloat(
					R.styleable.DragListView_slide_shuffle_speed, 0.75f)));

			mAnimate = mSlideRegionFrac > 0.0f;

			float frac = a.getFloat(R.styleable.DragListView_drag_scroll_start,
					mDragUpScrollStartFrac);

			setDragScrollStart(frac);

			mMaxScrollSpeed = a.getFloat(
					R.styleable.DragListView_max_drag_scroll_speed,
					mMaxScrollSpeed);

			removeAnimDuration = a.getInt(
					R.styleable.DragListView_remove_animation_duration,
					removeAnimDuration);

			dropAnimDuration = a.getInt(
					R.styleable.DragListView_drop_animation_duration,
					dropAnimDuration);

			boolean useDefault = a.getBoolean(
					R.styleable.DragListView_use_default_controller, true);

			if (useDefault) {
				boolean removeEnabled = a.getBoolean(
						R.styleable.DragListView_remove_enabled, false);
				int removeMode = a.getInt(R.styleable.DragListView_remove_mode,
						DragController.FLING_REMOVE);
				boolean sortEnabled = a.getBoolean(
						R.styleable.DragListView_sort_enabled, true);
				int dragInitMode = a.getInt(
						R.styleable.DragListView_drag_start_mode,
						DragController.ON_DOWN);
				int dragHandleId = a.getResourceId(
						R.styleable.DragListView_drag_handle_id, 0);
				int flingHandleId = a.getResourceId(
						R.styleable.DragListView_fling_handle_id, 0);
				int clickRemoveId = a.getResourceId(
						R.styleable.DragListView_click_remove_id, 0);
				int bgColor = a.getColor(
						R.styleable.DragListView_float_background_color,
						Color.BLACK);
