...
public class HardUpdateAdapter extends BaseAdapter<DataappInfo> {
	...
	private static int[] apkStat = new int[] { R.string.apkstat_0,
			R.string.apkstat_1, R.string.apkstat_2, R.string.apkstat_3 };
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (item != null) {
			holder.icon.setBackgroundDrawable(ApkUtils.getIconFromPackage(
					v.getContext(), item.info, item.localPath));
			holder.name.setText(ApkUtils.getLabelFromPackage(v.getContext(),
					item.info, item.localPath));
