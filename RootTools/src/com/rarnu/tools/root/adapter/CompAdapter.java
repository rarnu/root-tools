...
public class CompAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (item != null) {
			...
			String compName = item.component.className.substring(item.component.className.lastIndexOf(".") + 1);
			...
			holder.itemReceiverStatus.setTextColor(item.enabled ? 0xFF008000 : Color.RED);
			String ret = "";
			int i = 0;
			// Integer act;
			if (item.component instanceof PackageParser.Activity) {
						...
						for (PackageParser.ActivityIntentInfo aii : pa.intents) {
							if (aii.countActions() > 0) {
								for (i = 0; i < aii.countActions(); i++) {
									ret += aii.getAction(i).substring(aii.getAction(i).lastIndexOf(".") + 1)
