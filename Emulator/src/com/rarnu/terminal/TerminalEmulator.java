...
public class TerminalEmulator {
	...
	private static final int K_DECSC_DECRC_MASK = K_ORIGIN_MODE_MASK
			| K_WRAPAROUND_MODE_MASK;
	...
	public TerminalEmulator(TermSession session, Screen screen, int columns,
			int rows, ColorScheme scheme) {
		mSession = session;
		mScreen = screen;
		mRows = rows;
		mColumns = columns;
	...
	public void updateSize(int columns, int rows) {
		...
		if (!fastResize) {
			...
			charAtCursor = mScreen.getSelectedText(cursorColor, mCursorCol,
					mCursorRow, mCursorCol, mCursorRow);
	...
	private void doEscLeftSquareBracket(byte b) {
		...
		case 'J':
			...
			case 0:
				...
				blockClear(0, mCursorRow + 1, mColumns, mRows
						- (mCursorRow + 1));
				break;
	...
	private void emit(int c, int style) {
		...
		if (width == 0) {
			if (mJustWrapped) {
				mScreen.set(mColumns - mLastEmittedCharWidth, mCursorRow - 1,
						c, style);
			} else {
				mScreen.set(mCursorCol - mLastEmittedCharWidth, mCursorRow, c,
						style);
			}
		} else {
