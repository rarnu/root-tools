...
public class UnicodeTranscript {
	...
	public UnicodeTranscript(int columns, int totalRows, int screenRows,
			int defaultStyle) {
		mColumns = columns;
	...
	private void blockCopyLines(int src, int len, int shift) {
		...
		if (shift < 0) {
			for (int i = 0; i < len; ++i) {
				...
				mLineWrap[(dst + i) % totalRows] = mLineWrap[(src + i)
						% totalRows];
			}
		} else {
	...
	public void blockCopy(int sx, int sy, int w, int h, int dx, int dy) {
		...
		if (sy > dy) {
			for (int y = 0; y < h; y++) {
				...
				if (lines[srcRow] instanceof char[]
						&& lines[dstRow] instanceof char[]) {
	...
	public boolean getChar(int row, int column, int charIndex, char[] out,
			int offset) {
		if (row < -mActiveTranscriptRows || row > mScreenRows - 1) {
			throw new IllegalArgumentException();
		}
