	...
	public void setChar(int column, int codePoint) {
		...
		if (shift > 0) {
			if (spaceUsed + shift > text.length) {
				...
				System.arraycopy(text, pos + oldLen, newText, pos + newLen,
						spaceUsed - pos - oldLen);
				mText = text = newText;
			} else {
				System.arraycopy(text, pos + oldLen, text, pos + newLen,
						spaceUsed - pos - oldLen);
			}
		}
		...
		if (shift < 0) {
			System.arraycopy(text, pos + oldLen, text, pos + newLen, spaceUsed
					- pos - oldLen);
		}
		...
		if (oldCharWidth == 2 && charWidth == 1) {
			...
			if (spaceUsed + 1 > text.length) {
				...
				System.arraycopy(text, nextPos, newText, nextPos + 1, spaceUsed
						- nextPos);
				mText = text = newText;
			} else {
				System.arraycopy(text, nextPos, text, nextPos + 1, spaceUsed
						- nextPos);
			}
			text[nextPos] = ' ';
			...
			if (column == columns - 1) {
				...
				if (column + nextWidth + 1 < columns) {
					nextLen = findStartOfColumn(column + nextWidth + 1) + shift
							- nextPos;
				} else {
				...
				if (nextWidth == 2) {
					text[nextPos] = ' ';
					if (nextLen > 1) {
						System.arraycopy(text, nextPos + nextLen, text,
								nextPos + 1, spaceUsed - nextPos - nextLen);
						shift -= nextLen - 1;
						offset[0] -= nextLen - 1;
					}
				} else {
					System.arraycopy(text, nextPos + nextLen, text, nextPos,
							spaceUsed - nextPos - nextLen);
					shift -= nextLen;
