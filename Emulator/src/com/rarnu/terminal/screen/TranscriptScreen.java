...
public class TranscriptScreen implements Screen {
	...
	public final void drawText(int row, Canvas canvas, float x, float y,
		...
		if (line == null) {
			if (selx1 != selx2) {
				...
				Arrays.fill(blank, ' ');
				renderer.drawTextRun(canvas, x, y, selx1, selx2 - selx1, blank,
						0, 1, true, defaultStyle);
			} else if (cx != -1) {
		...
		while (column < columns) {
			...
			if (style != lastStyle || cursorStyle != lastCursorStyle
					|| (width > 0 && forceFlushRun)) {
				if (lastRunStart >= 0) {
					renderer.drawTextRun(canvas, x, y, lastRunStart, runWidth,
							line, lastRunStartIndex, index - lastRunStartIndex,
		...
		if (lastRunStart >= 0) {
			renderer.drawTextRun(canvas, x, y, lastRunStart, runWidth, line,
		...
		if (cx >= 0 && imeText.length() > 0) {
			...
			renderer.drawTextRun(canvas, x, y, imePosition, imeLength,
					imeText.toCharArray(), imeOffset, imeLength, true,
	...
	public String getTranscriptText(GrowableIntArray colors) {
		return internalGetTranscriptText(colors, 0,
				-mData.getActiveTranscriptRows(), mColumns, mScreenRows);
	}
	...
	public String getSelectedText(GrowableIntArray colors, int selX1,
			int selY1, int selX2, int selY2) {
		return internalGetTranscriptText(colors, selX1, selY1, selX2, selY2);
	}

	private String internalGetTranscriptText(GrowableIntArray colors,
		...
		for (int row = selY1; row <= selY2; row++) {
			...
			if (line == null) {
				if (!data.getLineWrap(row) && row < selY2
						&& row < mScreenRows - 1) {
					builder.append('\n');
					if (colors != null) {
						colors.append(0);
			...
			for (i = 0; i < lineLen && column < width; ++i) {
				...
				if (c == 0) {
					break;
						...
						|| ((rowColorBuffer != null) && (rowColorBuffer
								.get(column) != defaultColor))) {
					lastPrintingChar = i;
				}
