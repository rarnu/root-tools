...
public class TermKeyListener {
	...
	public int mapControlChar(int ch) {
		return mapControlChar(mHardwareControlKey || mControlKey.isActive(),
				mFnKey.isActive(), ch);
	}
	...
	public int mapControlChar(boolean control, boolean fn, int ch) {
		...
		if (control) {
			...
			if (result == 'w' || result == 'W') {
				...
				result = (char) (result + KEYCODE_OFFSET
						+ TermKeyListener.KEYCODE_F1 - 1);
			} else if (result == '0') {
	...
	public void keyDown(int keyCode, KeyEvent event, boolean appMode,
		...
		default: {
			...
			if ((result & KeyCharacterMap.COMBINING_ACCENT) != 0) {
				...
				mCombiningAccent = result
						& KeyCharacterMap.COMBINING_ACCENT_MASK;
				return;
			}
			if (mCombiningAccent != 0) {
				int unaccentedChar = result;
				result = KeyCharacterMap.getDeadChar(mCombiningAccent,
						unaccentedChar);
	...
	private void updateCursorMode() {
		mCursorMode = getCursorModeHelper(mCapKey,
				TextRenderer.MODE_SHIFT_SHIFT)
